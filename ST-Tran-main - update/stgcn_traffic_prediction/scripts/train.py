...
opt.model_filename = '{}/close={}-spatial={}-mode={}-c={}-s={}-FS={}-model_N={}-scptmodel_d={}-{}-{}-{}'.format(opt.save_dir,opt.close_size,opt.spatial,opt.mode,opt.c,opt.s,opt.FS,opt.model_N,
...
def train_epoch(data_type,epoch):
    ...
    for idx, (c, target) in enumerate(data):
        #print(idx,c.shape,target.shape)
        if data_type == 'train':
            scheduler(optimizer,i,epoch)
        optimizer.zero_grad()#清空过往梯度
        model.zero_grad()
        pred = model(c.float(),opt.mode,opt.c,opt.s,opt.FS)
        loss = criterion(pred.float(), target.cuda().float()[:,:,opt.flow])#计算预测值与实际标签之间的梯度
        #print(loss)
...
def predict(test_type='train'):
    ...
    for idx, (c,target) in enumerate(data):
        start = time.time()
        pred = torch.relu(best_model(c.float(),opt.mode,opt.c,opt.s,opt.FS))
        end = time.time()
        t += (end-start)
...
def train_valid_split(dataloader, test_size=0.3, shuffle=True, random_seed=0):
...
if __name__ == '__main__':
    ...
    x_train, y_train, x_test, y_test, mmn = load_data(path, opt.close_size,                                               opt.test_size)
    ...
    if os.path.isfile(best_model):
        ...
        model = T_STGCN(opt.close_size, external_size, opt.model_N, opt.k, opt.spatial,opt.c_model_d,opt.s_model_d,opt.p_model_d,opt.t_model_d).cuda()#加载transformer模型，.cuda()使其在GPU中运算。
    scheduler = LR_Scheduler(opt.lr_scheduler, lr, total_epochs, len(train_loader),warmup_epochs=opt.warmup)#定义学习率
    optimizer = optim.Adam(model.parameters(),lr,betas=(0.9, 0.98), eps=1e-9)

    if not os.path.isdir(opt.save_dir):
        raise Exception('%s is not a dir' % opt.save_dir)

    if opt.loss == 'l1':#选择损失函数
